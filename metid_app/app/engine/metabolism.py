...
class DeepLearningPredictor:
    ...
    def verify_smiles(smiles: str) -> Optional[Chem.Mol]:
        ...
        try:
            ...
            logger.debug("dl_mol_sanitize_exception")
            return None
        except rdchem.KekulizeException as exc:
            logger.debug("dl_mol_kekulize_exception")
            return None
        except Exception as exc:
            logger.debug("dl_mol_verify_error")
            return None
    ...
    def _transformer_beam_search(
        ...
        for transform in transformations[:top_k]:
            try:
                ...
                logger.debug("dl_transform_failed")
                continue
...
class ConsensusEngine:
    ...
    def merge_metabolites(
        ...
        logger.info("consensus_merge_complete")
        return merged, stats
...
def _run_sygma(
    ...
    for node in tree.to_list():
        ...
        try:
            ...
            logger.debug("sygma_sanitize_skip")
            continue
        except Exception as exc:
            logger.debug("sygma_mol_skip")
            continue

        try:
            smiles = Chem.MolToSmiles(node_mol, canonical=True)
        except Exception as exc:
            logger.debug("sygma_smiles_skip")
            continue
...
def predict(
    ...
    *,
    ...
    try:
        ...
        logger.error("sygma_pipeline_error")
    ...
    if enable_dl:
        ...
        try:
            ...
            logger.error("dl_pipeline_error")
    else:
